...
import (
	...
	"github.com/icemining/ethereum-pool/util"
)
...
func (r *RedisClient) WriteBlock(login, id string, params []string, diff, roundDiff int64, height uint64, window time.Duration) (bool, error) {
	...
	cmds, err := tx.Exec(func() error {
		...
		tx.Rename(r.formatKey("shares", "roundCurrentShares"), r.formatRoundShares(int64(height), params[0]))
		tx.Rename(r.formatKey("shares", "roundCurrentScores"), r.formatRoundScores(int64(height), params[0]))
	...
	if err != nil {
		...
		scoresMap, _ := cmds[11].(*redis.StringStringMapCmd).Result()
		totalScore := int64(0)
...
func (r *RedisClient) writeShare(tx *redis.Multi, ms, ts int64, login, id string, diff int64, score int64, expire time.Duration) {
	tx.HIncrBy(r.formatKey("shares", "roundCurrentShares"), login, diff)
	tx.HIncrBy(r.formatKey("shares", "roundCurrentScores"), login, score)
...
func (r *RedisClient) formatRoundShares(height int64, nonce string) string {
	return r.formatKey("roundShares", "round_shares"+strconv.FormatInt(height, 10), nonce)
}

func (r *RedisClient) formatRoundScores(height int64, nonce string) string {
	return r.formatKey("roundScores", "round_scores"+strconv.FormatInt(height, 10), nonce)
}
...
func (r *RedisClient) GetMinerStats(login string, maxPayments int64) (map[string]interface{}, error) {
	...
	cmds, err := tx.Exec(func() error {
		...
		tx.HGet(r.formatKey("shares", "roundCurrentShares"), login)
		tx.HGet(r.formatKey("shares", "roundCurrentScores"), login)
		return nil
	})
